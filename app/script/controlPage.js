...
const createBootstrapTable1 = (arrayObj, where) => {
  ...
  $('.intestazione1').append(`<th scope="col">#</th>`);

  keys.forEach((el, idx) => {
    if (el != 'id' && el != 'id_type' && el != 'id_genre' && el != 'img_src') {
      $('.intestazione1').append(
        `<th class="text-light" scope="col">${el}</th>`
      );
    }
  });
  ...
  arrayObj.forEach((el, idx) => {
    const tr = document.createElement('tr');
    $(tr)
      .addClass(`riga${idx + 10}`)
      .html(`<th scope="row" class="text-light">${idx + 11}</th>`)
      .appendTo('.table-body1');
  });

  const righe = $('.table-body1').children('tr');
  //console.log(righe);

  for (let a = 0; a < righe.length; a += 1) {
    for (key of keys) {
      const riga = document.getElementsByClassName(`riga${a + 10}`);
      //console.log(key);
      if (
        key != 'id' &&
        key != 'id_type' &&
  ...
  try {
    ...
    for (let a = 0; a < 3; a++) {
      ...
      if (es.status == 'success') {
        if (a == 0) {
          createBootstrapTable1(es.data, $('#es1'));
        } else if (a == 1) {
